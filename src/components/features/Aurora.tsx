...
vec3 getRampColor(vec3 colorStops[3], float factor) {
  const float pos0 = 0.0;
  const float pos1 = 0.5;
  const float pos2 = 1.0;
  int index = 0;
  if (factor < pos1) {
    index = 0;
  } else {
  ...
  float startPos = (index == 0) ? pos0 : pos1;
  float endPos = (index == 0) ? pos1 : pos2;
  vec3 startColor = colorStops[index];
  vec3 endColor = colorStops[index+1];
  float range = endPos - startPos;
  float lerpFactor = (factor - startPos) / range;
  return mix(startColor, endColor, lerpFactor);
}

void main() {
  vec2 uv = gl_FragCoord.xy / uResolution;
  
  vec3 rampColor = getRampColor(uColorStops, uv.x);
